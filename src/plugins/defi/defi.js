...
export class DeFi {
    ...
    _setTokens(_tokens) {
        ...
        this.skhToken = _tokens.find((_item) => _item.symbol === 'SKH');
    ...
    */
    ...
    canTokenBeDeposited(_token) {
        return _token && _token.isActive && _token.canDeposit && _token.symbol !== 'SKH';
    }
    ...
    canTokenBeTraded(_token) {
        ...
        return _token && _token.isActive && (_token.canTrade || _token.symbol === 'SKH');
     ...
     * @return {Promise<Number>}
     */
    async fetchSKHTokenPrice(_to = 'USD') {
        const data = await this.apolloClient.query({
            query: gql`
